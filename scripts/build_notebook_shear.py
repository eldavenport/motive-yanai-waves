"""Generate the shear vs. momentum-flux-divergence analysis notebooks.

Emits, for each model:
  notebooks/yanai_shear_divergence_tpose6.ipynb
  notebooks/yanai_shear_divergence_tpose24.ipynb

Question: is the vertical momentum-flux divergence (-d<u'w'>/dz, -d<v'w'>/dz) --
the wave force on the mean flow -- organised by the vertical shear of the flow?
For each model the analysis is done on the TOTAL signal (anomaly from the window
mean) and on the YANAI band (15-40 day band-pass), against zonal / meridional /
total shear, using two shear variants (mean background shear dU/dz and the
band-matched fluctuating shear). It reports Pearson/Spearman r with a first-pass
significance estimate -- the definitive significance test is left OPEN, to be
chosen after seeing the results.

Reuses scripts/shear.py, stats_corr.py, fluxes.py, wave_filter.py and the same
plotting idioms as build_notebook_t6.py. Only the load/normalise cell differs
between the models; every analysis cell is shared.
"""
...
# --------------------------------------------------------------------------- #
# model-specific configuration + load cell
# --------------------------------------------------------------------------- #
CONFIG = {
    'tpose6': {
        'io': 'tpose6_io',
        'title_line': "TPOSE6 (daily, 2012-09-01 .. 2013-06-30; no spin-up drop)",
        'fig_dir': f'{REPO}/figures/tpose6',
        'prefix': '',
        'cov3d': 'yanai_flux_cov3d_tp6.nc',
        'meanuv': 'yanai_meanUV_maps_tp6.nc',
        'dt_days': '1.0',
        'map_subset': 'True',       # cov3d grid is the map sub-domain slice
        'load': (
...
"times_pre = prof['time'].values                     # no spin-up drop\n"
"# WVEL already averaged to cell centers in this cache\n"
"RAW = {'U': prof['UVEL'].values, 'V': prof['VVEL'].values,\n"
"       'Wc': prof['WVEL'].values}                   # each (time,lon,lat,depth)\n"
"etm = io.edge_trim_mask(times_pre)                  # drop band-pass edge days\n"
        ),
    },
    'tpose24': {
        'io': 'tpose24_io',
        'title_line': "TPOSE24 (3-hourly, from 2012-10-01; first 8 days spin-up dropped)",
        'fig_dir': f'{REPO}/figures',
        'prefix': '',
        'cov3d': 'yanai_flux_cov3d_dt60.nc',
        'meanuv': 'yanai_meanUV_maps_dt60.nc',
        'dt_days': 'io.OUT_STEP_SEC / 86400.0',
        'map_subset': 'False',      # cov3d grid is the full domain
        'load': (
...
# --------------------------------------------------------------------------- #
# shared cells (identical for both models)
# --------------------------------------------------------------------------- #
def build_cells(model):
    cfg = CONFIG[model]
    cells = []

    cells.append(md(
"# Momentum-flux divergence vs. vertical shear — " + cfg['title_line'] + "\n"
"\n"
"Is the vertical **momentum-flux divergence** "
"$-\\partial_z\\langle u'w'\\rangle,\\,-\\partial_z\\langle v'w'\\rangle$ "
"(the wave force on the mean flow) *organised by* the **vertical shear** of the\n"
...
"**meridional** $\\partial_z V$ and **total** $|S|=\\sqrt{(\\partial_zU)^2+"
"(\\partial_zV)^2}$ shear, using two shear variants:\n"
"\n"
...
"CACHE   = io.CACHE_DIR\n"
"FIG_DIR = '" + cfg['fig_dir'] + "'\n"
"COV3D   = f'{CACHE}/" + cfg['cov3d'] + "'\n"
"MEANUV  = f'{CACHE}/" + cfg['meanuv'] + "'\n"
"DT_DAYS = " + cfg['dt_days'] + "\n"
"MAP_SUBSET = " + cfg['map_subset'] + "  # cov3d/meanUV grid is a sub-domain slice?\n"
...
"magnitudes: the **Yanai vertical momentum flux** "
"$|\\langle\\mathbf{u}'w'\\rangle|$, its **divergence** "
"$|{-}\\partial_z\\langle\\mathbf{u}'w'\\rangle|$, and the **background shear**\n"
...
def main():
    for model in ('tpose6', 'tpose24'):
        cells = build_cells(model)
        nb = {'cells': cells,
        ...
        out = os.path.join(REPO, 'notebooks',
                           f'yanai_shear_divergence_{model}.ipynb')
