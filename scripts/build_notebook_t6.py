"""Generate notebooks/yanai_flux_analysis_tpose6.ipynb.

Mirrors the TPOSE24 notebook but for TPOSE6: daily band-pass, no spin-up drop
(20-day filter edge-trim kept), profiles at 170W/140W/110W (the profile figures
loop over longitude), PHIHYD read from the cache (reconstructed from T/S), and
maps over the 175W-105W / 10S-10N sub-domain. Figures go to figures/tpose6/.
"""
...
cells.append(md("## (a) Time series — full-column depth-integrated flux, per longitude"))
...
cells.append(md("## (a2) Depth–time (Hovmöller) sections of the vertical fluxes, per longitude"))
...
cells.append(md("## (b) Mean profiles with standard deviation, per longitude"))
...
cells.append(md("### Energy-flux component profiles, per longitude"))
...
out = os.path.join(REPO, 'notebooks', 'yanai_flux_analysis_tpose6.ipynb')
